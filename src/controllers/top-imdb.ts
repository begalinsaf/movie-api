...
export const topImdb = async (req: Request, res: Response) => {
  try {
    const page = req.query.page || 1

    const data: TopImdbTypes = {
      currentPage: parseInt(String(page)),
      totalPages: 1,
      data: []
    }

    const resp = await axiosClient.get(`/top-imdb?type=all&page=${page}`);
